/* Emojeo Step 3 Population UI — Pass 42 auto-resume + batch/overall progress */
(()=>{'use strict';
...
async function start(auto=false){
 ...
 manualStop=false;run.disabled=true;stop.disabled=false;controller=new AbortController();
 try{
  ...
  const cp=globalThis.emojeoStep3Population.loadCheckpoint('step3-diverse-001'),d=cp?.results?.length||0,t=cp?.totalUnits||6399;lastRun=cp||lastRun;down.disabled=!cp;bar.style.width=`${pct(d,t)}%`;
...
function init(){
 ...
 $('step3Run').onclick=()=>start(false);$('step3Stop').onclick=()=>{manualStop=true;controller?.abort()};$('step3Download').disabled=!cp;$('step3Download').onclick=()=>dl(`emojeo-step3-${new Date().toISOString().replace(/[:.]/g,'-')}.json`,lastRun||globalThis.emojeoStep3Population.loadCheckpoint('step3-diverse-001'));
