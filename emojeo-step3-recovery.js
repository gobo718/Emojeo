...
const TARGET_SHAPE_VALIDATOR_VERSION=2;
...
function emojiPeerNameCandidates(v){const n=normalizePeerLabel(v),out=[n];if(n.endsWith(' emoji'))out.push(n.slice(0,-6).trim());return [...new Set(out.filter(Boolean))]}
function isEmojiPeerTarget(v){const raw=clean(v);if(!raw)return false;if(/\p{Extended_Pictographic}/u.test(raw))return true;if(/^U\+[0-9A-F]{4,6}(?:\s+U\+[0-9A-F]{4,6})*$/i.test(raw))return true;return emojiPeerNameCandidates(raw).some(n=>emojiPeerNames?.has(n))}
...
function validateRecoveryResult(result){if(!result||result?.targetShapeValidation?.version===TARGET_SHAPE_VALIDATOR_VERSION)return result;const current=Array.isArray(result.assertions)?result.assertions:[],priorTargetRejected=Array.isArray(result.targetShapeRejectedAssertions)?result.targetShapeRejectedAssertions:[],source=[],sourceSeen=new Set();for(const a of [...current,...priorTargetRejected]){const k=validationKey(a);if(!sourceSeen.has(k)){source.push(a);sourceSeen.add(k)}}const kept=[],invalid=[];for(const a of source){const reason=targetShapeReason(a);if(reason)invalid.push({...a,reconciliationReason:`Target-shape validator: ${reason}`,targetShapeValidator:`client-v${TARGET_SHAPE_VALIDATOR_VERSION}`});else{const cleanA={...a};delete cleanA.targetShapeValidator;if(clean(cleanA.reconciliationReason).startsWith('Target-shape validator:'))delete cleanA.reconciliationReason;kept.push(cleanA)}}const rejected=(Array.isArray(result.rejectedCandidates)?result.rejectedCandidates:[]).filter(a=>!clean(a?.targetShapeValidator).startsWith('client-v')),seen=new Set(rejected.map(validationKey));for(const a of invalid){const k=validationKey(a);if(!seen.has(k)){rejected.push(a);seen.add(k)}}const invalidTags=new Set(invalid.map(a=>clean(a.tag))),keptTags=new Set(kept.map(a=>clean(a.tag)));const priorRejectedTags=new Set(priorTargetRejected.map(a=>clean(a.tag)));const newTags=(Array.isArray(result.newTags)?result.newTags:[]).filter(t=>!invalidTags.has(clean(t))||keptTags.has(clean(t)));for(const a of kept){const tag=clean(a.tag);if(priorRejectedTags.has(tag)&&tag&&!newTags.some(t=>clean(t)===tag))newTags.push(a.tag)}const audit={version:TARGET_SHAPE_VALIDATOR_VERSION,mode:'client-post-reconciliation',checkedAt:new Date().toISOString(),checkedAssertionCount:source.length,rejectedCount:invalid.length,rejectedRelationships:[...new Set(invalid.map(a=>a.relationshipType))]};return {...result,assertions:kept,newTags,rejectedCandidates:rejected,targetShapeRejectedAssertions:invalid,targetShapeValidation:audit,reconciliation:{...(result.reconciliation||{}),targetShapeValidation:audit}}}
