/* Emojeo Step 3 Recovery Mapper — Pass 50
...
async function loadFile(file){input=JSON.parse(await file.text());if(!Array.isArray(input?.results))throw new Error('Selected file has no Step 3 results array');if(Number(input.completedUnits)!==Number(input.totalUnits))throw new Error(`This recovery pass expects a completed Step 3 export; file is ${input.completedUnits||0}/${input.totalUnits||0}`);spec=await fetch('Emojeo_STEP3_Diverse_Batch_001_RunSpec.json',{cache:'no-cache'}).then(r=>{if(!r.ok)throw new Error(`RunSpec load failed (${r.status})`);return r.json()});await loadEmojiPeerNames();const id=fingerprint(input,file),prior=await dbGet(id),subjects=groupSubjects(input);job=prior&&Array.isArray(prior.recovered)?{...prior,subjects,recovered:prior.recovered.map(validateRecoveryResult)}:{id,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),subjects,recovered:[]};await dbPut(job);const rejected=job.recovered.reduce((n,r)=>n+Number(r?.targetShapeValidation?.rejectedCount||0),0);setStatus(prior?`Loaded saved recovery checkpoint · ${job.recovered.length}/${subjects.length} subjects already recovered · target-shape validator audit: ${rejected} malformed accepted assertion(s) rejected and preserved for review.`:`Ready · completed Step 3 export verified · ${subjects.length} subjects found · first gate is exactly 1 subject.`);render()}
