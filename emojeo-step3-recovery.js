/* Emojeo Step 3 Recovery Mapper — Pass 46
...
const DB_NAME='emojeo-step3-recovery-v2',STORE='jobs';
...
function download(){if(!job||!input)return;const out={...input,recoverySchemaVersion:2,recoveryKind:'emojeo-step3-raw-notes-recovery',recoveryCreatedAt:new Date().toISOString(),recoveryStrategy:'one subject per wave; 857 relationships sharded across Mistral / GPT-4.1 mini / Qwen 3.7 Plus concurrently; notes-only normalization; strict PRESENT/UNCERTAIN parser; negative rows omitted; original results preserved; gated 1 -> 3 -> 10 -> full',recoveryResults:job.recovered};const blob=new Blob([JSON.stringify(out,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`emojeo-step3-recovered-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},1500)}
