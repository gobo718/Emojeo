/* Emojeo Step 4 Robust Sparse/Transpose Runner — Pass 67 */
(()=>{'use strict';
...
function assertionVerifyRecordId(aid){return `${JOB_ID}|verify67|${aid}`}
function verifiedCellMap(){
  const m=new Map();
  for(const r of verifyRecords){
    if(Array.isArray(r.cells)){for(const v of r.cells)m.set(cellKey(v.subjectIndex,v.assertionId),v)}
    else if(Number.isInteger(r.subjectIndex)){for(const v of (r.values||[]))m.set(cellKey(r.subjectIndex,v.assertionId),v)}
  }
  return m;
}
...
function parsePUN(text){
  const t=String(text||'');
  const part=label=>{const m=t.match(new RegExp(`\\\\b${label}\\\\s*[:=]\\\\s*\\\\[?([^\\\\];|\\\\n]*)\\\\]?`,'i'));return m?nums(m[1]):[]};
  const P=part('P'),U=part('U'),N=part('N');return {P,U,N,matched:/\\b[UPN]\\s*[:=]/i.test(t)};
}
...
async function verifyOneAssertion(aid,limit){
  ...
  return {id:assertionVerifyRecordId(aid),kind:'emojeo-step4-assertion-verify-result',jobId:JOB_ID,assertionId:aid,candidateSubjectIndexes:candidateIndexes,cells:candidateIndexes.map(si=>({subjectIndex:si,assertionId:aid,state:'uncertain',confidence:'low',evidence:`Pass 67 mechanical fail-safe after repeated provider/format failure: ${lastError}`,verificationParseMode:'provider-failure-fallback-uncertain'})),providerFailureFallback:true,completedAt:new Date().toISOString()};
}
