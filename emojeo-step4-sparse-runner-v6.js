/* Emojeo Step 4 Robust Sparse/Transpose Runner — Pass 69 */
(()=>{'use strict';
...
function render(){
  ...
  $('summary').innerHTML=`<span class="good">${screened.toLocaleString()}/1,278 assertions screened</span> · ${cand.size.toLocaleString()} sparse candidate cells · ${verified.toLocaleString()} genuinely verified · ${pendingCount.toLocaleString()} provider-failure pending · 4-way concurrency`;
...
function cellVerifyRecordId(si,aid){return `${JOB_ID}|verify69|${si}|${aid}`}
function isValidVerifiedCell(v){
  return Boolean(v && v.verificationParseMode &&
    v.verificationParseMode!=='provider-failure-fallback-uncertain' &&
    v.verificationParseMode!=='provider-failure-pending');
}
function verifiedCellMap(){
  const m=new Map();
  for(const r of verifyRecords){
    if(Array.isArray(r.cells)){
      for(const v of r.cells)if(isValidVerifiedCell(v))m.set(cellKey(v.subjectIndex,v.assertionId),v);
    }else if(Number.isInteger(r.subjectIndex)){
      for(const v of (r.values||[]))if(isValidVerifiedCell(v)||!v.verificationParseMode)m.set(cellKey(r.subjectIndex,v.assertionId),v);
    }
  }
  return m;
}
function pendingProviderFailureMap(){
  const m=new Map();
  for(const r of verifyRecords){
    if(r?.kind!=='emojeo-step4-cell-verify-result-v69')continue;
    const v=r.value;
    if(v?.verificationParseMode==='provider-failure-pending')m.set(cellKey(r.subjectIndex,r.assertionId),v);
  }
  return m;
}
function cellVerificationPrompt(subject,a,attempt){
  return [
    'EMOJEO STEP 4 SINGLE-CELL VERIFICATION',
    `Emoji: ${subject.glyph} ${subject.name}`,
    `Fixed assertion: ${a.assertionId}|${a.relationshipType}|${a.tag}`,
    '',
    'Evaluate ONLY whether this exact fixed assertion applies to this exact emoji.',
    'Do not rewrite the relationship type. Do not rewrite or substitute the tag.',
    '',
    ...
    'The observation description MUST begin with exactly one of:',
    'STATE=PRESENT',
    'STATE=ABSENT',
    'STATE=UNCERTAIN',
    'STATE=NOT_EVALUATED',
    'Then add "; EVIDENCE=" followed by one concise reason.',
    '',
    'PRESENT = directly supported by inherent, conventional, established, symbolic, metaphorical, or cultural semantics.',
    'ABSENT = meaningfully evaluable and does not apply.',
    'UNCERTAIN = genuine plausible support exists but evidence is ambiguous or context-dependent.',
    'NOT_EVALUATED = cannot responsibly judge without inventing outside context.',
    '',
    `Attempt ${attempt}/3. Another program reads STATE mechanically, so preserve that exact label.`
  ].join('\n');
}
function parseCellVerification(env,subject,a){
  ...
  for(const c of chunks){
    ...
    return {
      ...
      verificationParseMode:'single-cell-native-observation'
    };
  }
  return null;
}
async function verifyOneCell(si,aid){
  ...
  for(let attempt=1;attempt<=3;attempt++){
    try{
      const env=await callSemantic({
        schemaVersion:1,
        kind:'emojeo-step4-single-cell-verify-v69',
        subject:{id:`${subject.glyph}:${subject.name}`,glyph:subject.glyph,name:subject.name},
        domain:'Step 4 single-cell fixed assertion verification',
      ...
      if(parsed){
        return {
          ...
          kind:'emojeo-step4-cell-verify-result-v69',
  ...
  return {
    ...
    value:{
      ...
      verificationParseMode:'provider-failure-pending'
    },
...
async function runVerify(limit){
  ...
  await runPool(tasks,async t=>{
    ...
    // Replace any earlier Pass 69 record for the same cell in memory.
